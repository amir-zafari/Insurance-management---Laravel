@section('titel', 'افزودن کارفرما')

<div>
    <div class="header-area" id="headerArea">
        <div class="container">
            <!-- Header Content -->
            <div class="header-content position-relative d-flex align-items-center justify-content-between">
                <!-- Back Button -->
                <div class="back-button"><a href="/karfarma">
                        <svg class="bi bi-arrow-left-short" width="32" height="32" viewBox="0 0 16 16"
                             fill="currentColor"
                             xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd"
                                  d="M12 8a.5.5 0 0 1-.5.5H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5H11.5a.5.5 0 0 1 .5.5z"></path>
                        </svg>
                    </a></div>
                <!-- Page Title -->
                <div class="page-heading">
                    <h6 class="mb-0">افزودن کارفرما</h6>
                </div>
                <!-- Settings -->
                <div class="navbar--toggler" id="affanNavbarToggler" data-bs-toggle="offcanvas"
                     data-bs-target="#affanOffcanvas" aria-controls="affanOffcanvas"><span class="d-block"></span><span
                        class="d-block"></span><span class="d-block"></span></div>
            </div>
        </div>
    </div>

    <div class="page-content-wrapper py-1">
        <div class="container">
            <div class="card">
                <div class="card-body">
                    <div class="direction" >
                        <a class="btn m-1 btn-success" href="
{{ route('karfarma.create') }}
                            ">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus-fill me-2" viewBox="0 0 16 16">
                                <path d="M1 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
                                <path fill-rule="evenodd" d="M13.5 5a.5.5 0 0 1 .5.5V7h1.5a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0V8h-1.5a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5z"/>
                            </svg>
                            افزودن</a>
                        <a class="btn m-1 btn-danger" href="
{{ route('karfarma.trashed') }}
                            ">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus-fill me-2" viewBox="0 0 16 16">
                                <path d="M1 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
                                <path fill-rule="evenodd" d="M13.5 5a.5.5 0 0 1 .5.5V7h1.5a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0V8h-1.5a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5z"/>
                            </svg>
                            ({{\App\Models\karfarma::onlyTrashed()->count()}})  سطل زباله</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="pt-3"></div>
        <div class="container">
            <div class="card">
                <div class="card-body">
                    @include('errors.error')
                    <form wire:submit.prevent="karfarmaForm"

                          enctype="multipart/form-data" role="form">
                        <div class="input-group mb-3"><span class="input-group-text">نام کامل</span>
                            <input class="form-control" wire:model.lazy="karfarma.name"  type="text" aria-label="First name" placeholder="نام">
                        </div>
                        <div class="input-group mb-3"><span class="input-group-text" id="basic-addon1">شماره تماس
                                </span>
                            <input class="form-control"  wire:model.lazy="karfarma.somare"  type="text" placeholder="شماره مبایل" aria-label="Username"
                                   aria-describedby="basic-addon1">
                        </div>
                        <div class="input-group mb-3"><span class="input-group-text" id="basic-addon1">کد گارگاه
                                </span>
                            <input class="form-control"   wire:model.lazy="karfarma.karkah"  type="text"  placeholder="کد گارگاه" aria-label="Username"
                                   aria-describedby="basic-addon1">
                        </div>
                        <div class="input-group"><span class="input-group-text">یاداشت</span>
                            <textarea class="form-control"  wire:model.lazy="karfarma.text"  aria-label="With textarea"
                                      placeholder="........."></textarea>
                        </div>
                        <div class="pt-3"></div>
                        <div class="form-check form-switch">
                            <input class="form-check-input form-check-warning" wire:model.lazy="karfarma.gardsh" id="warningSwitch" type="checkbox" checked>
                            <label class="form-check-label" for="warningSwitch">عملیات گردش حقوق</label>
                        </div>
                        <div class="pt-3"></div>
                        <button class="btn btn-primary w-100 d-flex align-items-center justify-content-center"
                                type="submit">افزودن
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus-fill me-2" viewBox="0 0 16 16">
                                <path d="M1 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
                                <path fill-rule="evenodd" d="M13.5 5a.5.5 0 0 1 .5.5V7h1.5a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0V8h-1.5a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5z"/>
                            </svg>
                        </button>

                    </form>
                </div>
            </div>
        </div>
    </div>

</div>
